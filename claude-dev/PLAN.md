...
| 2026-04-13 | License changed from CC BY 4.0 to dual-license: CC BY-SA 4.0 + commercial | CC BY 4.0 allowed anyone to take curated configs, modules, and tooling and bundle them into proprietary products with only a buried attribution line. CC BY-SA 4.0 adds ShareAlike: derivatives must remain open under the same or compatible terms. Dual-license option allows organizations needing proprietary use to obtain a commercial license from Cutaway Security, LLC. Does not block legitimate use by MSSPs, consultants, or training -- only proprietary product bundling without SA compliance. NOTICE file added with explicit attribution requirements. Copyright headers added to all configs, modules, tools. |
| 2026-04-13 | VM tracking consolidated to `~/.ssh/config` as single source of truth; `dev-inventory.csv`, `remote-testing.local.conf`, `remote-testing.example.conf`, and `proxmox-api.conf` eliminated | Audit of real usage showed all three planned tracking files duplicated data already in `~/.ssh/config` (host alias, IP, user, key path, Proxmox host). Only VMID was uniquely in the CSV, and it was also in SSH config as free-form comments. Proxmox API file was never used -- direct SSH to Proxmox host was the actual lifecycle channel. Standardized VMID comment format `# VMID=<id> PROXMOX=<alias>` makes SSH config machine-parseable for helper scripts. OS/version metadata stays in `TESTING_STANDARD.md` Section 2.1 table (it is documentation, not connection data). Net effect: one file to maintain, zero drift between sources, no per-project local conf needed. Pattern is portable to any project that tests against a Proxmox VM fleet. |
| 2026-04-14 | Release process gets a helper script (`claude-dev/release.sh`) that automates Steps 1-5 and stops before any push to main | Post-v8 review of `GIT_RELEASE_STEPS.md` found three staleness issues (Overview's export-ignore list missing 4 of 7 paths, Pre-Release Checklist missing NOTICE, "Files Removed During Release" table missing GITHUB_ISSUE_STANDARD.md) and revealed the manual nine-step process was prone to copy-paste errors. Chose Option A (minimal, local bash script) over Option B (Claude-in-the-loop) because releases happen roughly once per phase and the workflow benefits from being boring and predictable. Force-push to main and tagging `v#` on main are explicitly not automated -- the cost of a typo on those two commands is catastrophic and the benefit of automating them is near zero. Script includes website-only detection: if all changes since the prior `dev-v*` tag are under `docs/`, it recommends `deploy-site.sh` instead of cutting a full release. Dual-tag system (`dev-v#` on claude-dev, `v#` on main) confirmed useful -- `dev-v#` preserves the full dev state that produced a release (planning docs, standards, claude-dev/ contents) which would otherwise be unrecoverable as claude-dev moves forward. |
